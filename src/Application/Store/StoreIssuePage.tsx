...
import type { StockRequest, StockRequestLine } from '@/Application/StockRequest/types';

const STATUS_FILTERS = ['Pending', 'Partially Issued', 'Issued', 'Rejected', 'Cancelled'] as const;

const StoreIssuePage: React.FC = () => {
  const { canEdit } = usePermissions();
  const canIssue = canEdit('StoreIssuePage');
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.error ?? 'Failed to load store requisitions');
    } finally {
      setLoading(false);
    }
  }, [statusFilter]);
  ...
  // ── Real-time: new / changed requisitions while this screen is open ──────
  useEffect(() => {
    if (!socket) return;
    socket.emit(SOCKET_JOIN_INVENTORY);

    const onRequestUpdated = (payload: { request?: StockRequest; action?: string }) => {
      if (payload?.action === 'created') toast.info(`New store requisition ${payload.request?.request_no ?? ''}`);
      fetchRequests();
    };
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      toast.success(`Requisition ${selected.request_no} rejected`);
      setRejectOpen(false);
      setRejectReason('');
      setSelected(null);
      fetchRequests();
  ...
  return (
    <div className="flex flex-col min-h-full bg-muted/20">
      <PageHeader
        icon={PackageCheck}
        title="Store Issue"
        description="Review store requisitions and issue items — issuing reduces live inventory"
      >
        <Button
          variant="outline"
          size="sm"
