        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs text-muted-foreground">
              ...
              {grn.doc_ref_no && (
                <div>
                  <span className="text-muted-foreground/70">Doc Ref: </span>
                  <span className="font-medium">{grn.doc_ref_no}</span>
                </div>
              )}
              {grn.challan_no && (
                <div>
                  <span className="text-muted-foreground/70">Challan: </span>
                  <span>{grn.challan_no}</span>
                </div>
              )}
              {grn.vehicle_no && (
                <div>
                  <span className="text-muted-foreground/70">Vehicle: </span>
                ...
                {grn.items.map((item, idx) => (
                  <TableRow key={idx}>
                    ...
                    <TableCell>
                      ...
                      {item.specification && (
                        <p className="text-xs text-muted-foreground/70">{item.specification}</p>
                      )}
                    </TableCell>
                    ...
                    <TableCell>
                      <Badge className={`text-xs ${conditionColor[item.condition] ?? 'bg-muted text-muted-foreground'}`}>
                        {item.condition}
                      </Badge>
                    </TableCell>
