  ...
  return (
    <div className="w-72 flex-shrink-0 bg-card border-r flex flex-col overflow-hidden">
      ...
      <div className="px-3 py-2.5 border-b flex items-center justify-between">
        ...
        {onAddNew && (
          <Button size="sm" className="h-7 text-xs bg-primary hover:bg-primary/90" onClick={onAddNew}>
      ...
      {/* Stats */}
      <div className="px-3 py-2 border-b bg-muted/40 flex gap-3 text-xs font-medium flex-wrap">
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-0.5">
                  <span className="text-xs font-mono text-primary font-semibold">{item.item_code}</span>
                  <Badge className={`text-xs shrink-0 ml-1 py-0 ${statusBadge[color]}`}>{label}</Badge>
                </div>
                <div className="text-sm font-medium text-foreground truncate">{item.item_name}</div>
                <div className="flex items-center justify-between mt-1">
                  <span className="text-xs text-muted-foreground/70">{item.category}</span>
                  <span className="text-xs font-semibold text-foreground">
                    {item.current_stock} {item.uom}
                    <ChevronRight size={12} className="inline ml-0.5 text-muted-foreground/70" />
                  </span>
                </div>
              </button>
            );
